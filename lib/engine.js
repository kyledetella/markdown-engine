...
var DEFAULT_LAYOUT_PATH = 'views/layout.html';
var DEFAULT_PARTIAL_DIR = 'views/partials'
var CACHING_ENABLED = process.env.NODE_ENV === 'production';
var layoutCache = {};
var templateCache = {};
...
function getLayoutPath() {
  return path.resolve(basePath ? basePath + DEFAULT_LAYOUT_PATH : DEFAULT_LAYOUT_PATH);
}

function getPartialPath(pathname) {
  var partialDir = basePath ? basePath + DEFAULT_PARTIAL_DIR : DEFAULT_PARTIAL_DIR;
...
function renderMarkdownTemplate(filePath, options, callback) {
  ...
  if (templateCache[filePath] && CACHING_ENABLED) {
    hydrate(templateCache[filePath], options).then(function (html) {
      callback(null, html);
    }, function (err) {
      callback(err);
    });
    ...
    fs.readFile(filePath, 'utf8', function (err, str) {
      ...
      try {
        ...
        templateCache[filePath] = templateHtml;
...
function continueConfiguration(filePath, options, fn) {
  ...
  renderMarkdownTemplate(filePath, options, function (err, templateHtml) {
    if (err) {
      ...
      if (layoutCache[layoutTemplatePath] && CACHING_ENABLED) {
        layoutHtml = layoutCache[layoutTemplatePath];
        ...
        fs.readFile(layoutTemplatePath, 'utf8', function (err, html) {
          ...
          layoutCache[layoutTemplatePath] = html;
